...
/**
 * The four transactional emails, as pure functions of the data they show.
 *
...
export function buildOrderEmail(kind: OrderEmailKind, data: OrderEmailData): MailMessage {
  switch (kind) {
...
function textItems(d: OrderEmailData): string[] {
  return [
    'IN THIS ORDER',
    ...d.lines.map(
      (l) => `  ${l.name}${l.size ? ` · ${l.size}` : ''} × ${l.quantity}   ${l.lineTotal}`,
    ),
...
function textAddress(d: OrderEmailData): string[] {
  return d.address.length ? ['DELIVERING TO', ...d.address.map((line) => `  ${line}`)] : [];
}
